...
- **[词法定义](lexical/index.md#title)**: 包含注释规范、字面量（数字、字符串）、标识符命名规则及符号。
- **[语法核心](grammar/index.md#title)**: 详细描述变量声明、函数类型（`def`/`fun`/`act`）、控制流、类型系统及类与对象。
- **[宏系统](macro/index.md)**: 介绍 Lumos 的元编程能力，包括编译期配置、代码生成及正则表达式支持。

## 设计哲学摘要 {#design-summary}

Lumos 的语法设计遵循以下原则：

1. **明确性胜过隐晦**: 副作用必须通过 `act` 和权限标记显式声明。
2. **安全但不设限**: 提供 `effectful` 作为受约束的逃生舱（如 `effectful fun` 需权限列表与回滚块），但在常规代码中保持高度安全性。
