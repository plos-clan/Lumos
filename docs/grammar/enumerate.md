...
### 与 `variant` 的区别 {#enum-vs-variant}

- `enum`（含 `as table`）用于离散标签及其附属元数据映射；
- `variant` 用于“分支 + 异构载荷”的运行时值建模。

如果目标是表示“当前只会是某一个分支，且分支携带不同类型数据”，应使用 `variant`；  
如果目标是“同一个枚举标签可映射到多列静态属性”，应使用 `enum as table`。
