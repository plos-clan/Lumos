...
使用 `%` 占位符处理高阶函数，实现效应透明转发。`def[%]`、`fun[%]`、`act[%]` 均支持效应多态：带 `[%]` 的函数可接受任意纯度的闭包参数（`def`、`fun`、`act`），副作用完全在**闭包生成处**和**调用方**两处限制，高阶函数体内无需感知。
...
|---------|------------|--------------|--------------|-----------------|
| `def[%] foo(f)` | `def`（零效应）| `def` | `fun` | `act[P]` |
| `fun[%] foo(f)` | `fun` | `fun` | `fun` | `act[P]` |
