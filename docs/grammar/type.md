...
更多信息见[字符串字面量](../lexical/string.md) 。
...
### 位置与偏移类型 {#index-offset-types}

为提升数组、切片、指针相关操作的语义清晰度，Lumos 在语义层区分：

- `index`：索引类型，表示容器内位置，取值范围为 `[0, len)`（包含 `0`，不包含 `len`），不允许负数。
- `offset`：偏移类型，表示相对位移，可正可负，主要用于指针与迭代器移动。
...
`index` 与 `offset` 在语言层是语义化类型（非普通别名）；兼容期可通过显式转换与 `usize`/`isize` 互通。
...
- `index <-> usize`、`offset <-> isize` 默认要求显式转换。
- 禁止 `index` 与 `offset` 隐式混用，避免符号与截断问题。

#### 切片与区间规则

- 切片区间边界统一使用 `index`。
- 若支持“从尾部偏移”语义，应使用独立语法，不与普通 `index` 混用。

#### 兼容期策略

在迁移阶段，标准库可以保留 `usize`/`isize` 旧签名作为兼容重载，但新文档与新接口应优先标注 `index`/`offset`。
...
- `[]T`：切片，记录长度，支持 `.len` 属性，访问元素使用 `s[i]`（`i` 为 `index`），支持安全的边界检查。

切片区间的起止边界统一使用 `index` 语义；若提供“从尾部偏移”能力，应使用独立语法并按 `offset` 语义处理。
