...
#### 编译期整数推导 {#compile-time-integer-inference}
...
#### 编译期浮点推导 {#compile-time-float-inference}
...
## 变量声明规范 {#variable-declaration-rules}
...
## 类型内置成员 {#type-builtins}

所有数字类型都拥有一系列内置的静态成员，用于获取该类型的元数据和常用常量。

### 数字类型共有成员 {#numeric-common-members}
...
相关内容：单位系统见 [单位系统](measure.md#measure-measure)。
