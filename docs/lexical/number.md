...
- 常规整数形式：`123`、`123_456`
- 附带类型后缀示例：`123_i32` 类型后缀必须小写且前方带下划线。

### 整数前缀 {#integer-prefixes}
...
- `0x` 或 `0h`：十六进制
- `0d`：十进制
- `0f`：十六进制按位表示的浮点数（例如 `0f_0000_0000` 表示 `0.0_f32`）
  > 此为特例，必须为 32 64 位等受到支持浮点数的按位表示，不得缺位或多位，按照十六进制整数解析然后保持位表示不变地转换为对应的浮点数。

有进制前缀的字面量必须为整数；其他无前缀的数字可视为十进制并可能为浮点。

### 整数后缀 {#integer-suffixes}

#### 整数虚数后缀 {#imaginary-integer-suffixes}

- `i`、`j`、`k` 见 [虚数、四元数](#imaginary-quaternions)

#### 整数进制后缀 {#base-suffixes}
...
#### 浮点虚数后缀 {#imaginary-float-suffixes}
