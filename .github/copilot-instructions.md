...
- `act`: **副作用函数**。允许 IO、全局状态修改等。可以调用所有类型函数。必须声明权限（如 `act[net] foo()`）。
- **修饰符与标记**:
  - `async fun` / `async act`: 异步函数。`async fun` 保持逻辑纯。
...
- `measure`: 用于定义计量单位（如 `meter`, `kg`），避免与 `unit` 类型混淆。
...
- `dbg(...)`: 调试伪函数。全场景可用（包括 `def`），编译期或运行时向 `stderr` 输出信息。
- **权限系统**: `act` 函数必须声明权限。支持 `yields` 传播权限，`%` 效应多态。使用 `using ... with [perm]` 限制模块配额。支持 `permission ... aka ...` 定义别名。内置权限采用层级命名（如 `io.out`, `fs.read`）。
